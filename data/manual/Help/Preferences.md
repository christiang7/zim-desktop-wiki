...
**Remember window placement** if enabled, zim will remember the window placement of the main window and overrule the window manager on startup to place the window in the same location. In several cases this may mis-behave and it is better to let the window manager take care of placement. Notably, this option may place the window in an invisible location when switching between multi-monitor and single-monitor setups.
